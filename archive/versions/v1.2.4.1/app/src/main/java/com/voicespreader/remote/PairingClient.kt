...
class PairingClient {
    ...
    fun connect(
        ...
        onMicrophoneCommand: (Boolean, Long) -> Unit,
                ...
                while (generation == synchronized(connectionLock) { connectionGeneration }) {
                    ...
                    when (type) {
                        10 -> {
                            ...
                            val enabled = framedInput.readUnsignedByte() != 0
                            remaining--
                            val commandId = if (remaining >= Long.SIZE_BYTES) {
                                framedInput.readLong().also { remaining -= Long.SIZE_BYTES }
                            } else {
                                0L
                            }
                        ...
                        13 -> {
                            if (remaining != 0) error("电脑断开通知帧无效")
                            onRemoteDisconnect()
                        }
                    }
                    if (remaining > 0) {
